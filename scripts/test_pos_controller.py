...
def correct_orientation_only(pose: Pose) -> Pose:
    """只校正姿态方向，使其从 hand_tcp 转为 link8，不改变位置"""
    q = R.from_quat([
        pose.orientation.x,
        pose.orientation.y,
        pose.orientation.z,
    ...
    pose_corrected = Pose()
    pose_corrected.position = pose.position  # 保持不变
...
def main():
    ...
    # take 1/3 of imitated dq
    start_idx = len(imitated_dqs) // 2
    ...
    for dq in full_dqs:
        ...
        pose = correct_orientation_only(pose)
        group.set_pose_target(pose)
        success, plan, _, _ = group.plan()
        if success:
            q = plan.joint_trajectory.points[-1].positions
    ...
    transition = jtraj(curr_q, q_dense[0], 4000).q
    transition_dq = jtraj(curr_q, q_dense[0], 4000).qd

    q_dense = list(transition) + q_dense
    qd_dense = list(transition_dq) + vel_dense
    ...
    cmd_pub = rospy.Publisher('/joint_position_example_controller/command', JointTrajectory, queue_size=1)
    ...
    def send_commands():
        print("Sending full trajectory...")
        panda = rtb.models.DH.Panda()
        panda.tool = SE3(0, 0, 0.107)

        traj_msg.header.stamp = rospy.Time.now()  # Set header timestamp

        for i, q in enumerate(q_dense):
            pt = JointTrajectoryPoint()
            pt.positions = list(q[:7])  # Ensure exactly 7 joint values
            pt.time_from_start = rospy.Duration(i * 0.001)  # 2ms steps (500 Hz)
            traj_msg.points.append(pt)

        cmd_pub.publish(traj_msg)  # ✅ Only publish ONCE
        print("✅ Trajectory published with", len(traj_msg.points), "points.")
