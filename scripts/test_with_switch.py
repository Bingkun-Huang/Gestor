...
def main():
    ...
    '''
    orig_q = demo_dqs[-1].q_r    # pyquaternion 格式：Quaternion([w, x, y, z])

# 2. 定义一个轴和角度（这里绕 Z 轴 45°）
    axis = np.array([0, 0, 1])
    angle_deg = 45
    angle_rad = np.deg2rad(angle_deg)

# 3. 构造绕该轴的四元数（pyquaternion 接受 axis–angle 构造器参数是 axis 向量和弧度）
    offset_q = Quaternion(axis=axis, angle=angle_rad)  

# 4. 组合旋转：你想先做示教末帧旋转再做 offset 旋转，就右乘
#    （如果想先绕新轴再回到示教末帧方向，就左乘）
    goal_rot = orig_q * offset_q

    #goal_rot = demo_dqs[-1].q_r # copy from last demo dq

    goal_trans = [0.4, -0.2, 0.3]
